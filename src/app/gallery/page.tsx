...
export default function GalleryPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 md:px-8 text-parchment flex flex-col gap-10 font-jakarta">
      ...
      <div className="flex flex-wrap gap-2.5 justify-center border-b border-gold-primary/5 pb-5 select-none">
        {categories.map((cat) => {
          ...
          const isSelected =
            ...
            (cat === "All" && selectedCategory === "എല്ലാം") ||
            (cat === "എല്ലാം" && selectedCategory === "All");
      ...
      {loading ? (
          ...
          {images.map((img) => (
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 right-5 flex flex-col gap-1 z-10">
                <span className="text-[9px] uppercase font-bold text-gold-primary tracking-widest">
                  {img.category}
                </span>
        ...
        <div className="text-center py-20 border border-dashed border-gold-primary/10 rounded-xl bg-surface/30 max-w-md mx-auto p-6">
          <p className="text-mutedText text-xs">
            No heritage images found in this category.
          </p>
        </div>
      )}
      ...
      {/* Modal */}
      {selectedImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 md:p-6 bg-background/90 backdrop-blur-sm">
          <div className="relative w-full max-w-4xl max-h-[90vh] rounded-xl border border-gold-primary/25 bg-surface overflow-hidden shadow-2xl flex flex-col md:flex-row">
            ...
            {/* Image */}
            <div className="w-full md:w-3/5 flex items-center justify-center bg-black/40">
              <img
            ...
            {/* Info */}
            <div className="w-full md:w-2/5 p-6 md:p-8 flex flex-col justify-between gap-6 overflow-y-auto">
              ...
              <div className="flex flex-col gap-4">
                ...
                <h2 className="font-cinzel text-lg md:text-xl font-bold text-gold-primary">
                  {isMl
                    ? selectedImage.titleMalayalam
                    : selectedImage.title}
                </h2>

                <div className="w-8 h-[1px] bg-gold-primary/30" />

                <p className="text-mutedText text-xs md:text-sm leading-relaxed whitespace-pre-line">
                  {isMl
                    ? selectedImage.descriptionMalayalam
              ...
              <div className="flex flex-col gap-4">
                ...
                <div className="flex items-center gap-1.5 text-[10px] text-mutedText/85">
                  <Archive size={12} className="text-gold-primary" />
                  preserve of Thozhiyur See
                </div>
              </div>
